...
type AgentResult = { mode: string; status?: string; transactionHash?: string; decision: { decision: string; asset_id: string; route_id: string; weight: number; reason_codes: string[] } | null; decisionHash?: string; reason?: string };
...
export default function Home() {
  ...
  return <main>
    ...
    {agent && <section className="result"><p className="eyebrow">GENLAYER DECISION</p><h2>{agent.decision?.decision ?? "PENDING"}</h2><p className="muted">{agent.mode === "genlayer" ? `Studionet consensus · ${agent.status ?? "submitted"}` : agent.reason}</p>{agent.decision && <div className="decision-grid"><span>Asset <b>{agent.decision.asset_id}</b></span><span>Route <b>{agent.decision.route_id}</b></span><span>Weight <b>{(agent.decision.weight / 1e16).toFixed(1)}%</b></span><span>Reasons <b>{agent.decision.reason_codes.join(" · ")}</b></span></div>}{agent.transactionHash && <p className="muted">Transaction: {agent.transactionHash.slice(0, 20)}…</p>}</section>}
