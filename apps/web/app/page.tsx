...
type AgentOption = { asset_id: string; route_id: string; weight: number | string; reason_codes: string[] };
type AgentResult = { requestId?: string; mode: string; status?: string; transactionHash?: string; decision: { decision: string; asset_id: string; route_id: string; weight: number | string; reason_codes: string[]; options?: AgentOption[] } | null; decisionHash?: string; reason?: string };
...
export default function Home() {
  ...
  return <main>
    ...
    {agent && <section className="result"><p className="eyebrow">GENLAYER PORTFOLIO OPTIONS</p><h2>{agent.decision?.decision ?? "PENDING"}</h2><p className="muted">{agent.mode === "genlayer" ? `Studionet consensus · ${agent.status ?? "submitted"}` : agent.reason}</p>{agent.decision?.options?.length ? <div className="allocations">{agent.decision.options.map((option) => { const asset = result?.assets.find((item) => item.id === option.asset_id); const weight = Number(option.weight); return <article className="allocation" key={`${option.asset_id}-${option.route_id}`}><div><strong>{asset?.symbol ?? option.asset_id} · {asset?.name}</strong><p>{asset?.sector ?? "Verified Base asset"}</p><p>{option.reason_codes.join(" · ")}</p><button className="buy" onClick={() => result && preparePurchase({ assetId: option.asset_id, weight: weight / 1e18, reason: option.reason_codes.join(" · ") })}>Review purchase</button></div><b>{(weight / 1e16).toFixed(1)}%</b></article>; })}</div> : agent.decision && <div className="decision-grid"><span>Asset <b>{agent.decision.asset_id}</b></span><span>Route <b>{agent.decision.route_id}</b></span><span>Weight <b>{(Number(agent.decision.weight) / 1e16).toFixed(1)}%</b></span><span>Reasons <b>{agent.decision.reason_codes.join(" · ")}</b></span></div>}{agent.transactionHash && <><p className="muted">Transaction: {agent.transactionHash.slice(0, 20)}…</p><button className="poll" onClick={pollAgent}>Refresh GenLayer status</button></>}</section>}
