...
type Route = { id: string; assetId: string; venueId: string; status: string; liquidityUsd: number | null; priceImpactBps: number | null; reason: string };
...
export default function Home() {
  ...
  async function preparePurchase(allocation: Result["allocations"][number]) {
    ...
    const response = await fetch("/api/execution/validate", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ instructionId: crypto.randomUUID(), account: wallet.address, tokenIn: "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913", tokenOut: asset.contractAddress, venueId: route.venueId, amountIn: "0", minimumAmountOut: "0", deadline: Math.floor(Date.now() / 1000) + 300 }) });
    const body = await response.json();
    setPurchaseStatus(body.valid ? "Purchase request validated for review." : `Purchase blocked: ${(body.reasons ?? []).join("; ")}`);
  }
  ...
  return <main>
    <header><span className="mark">SP</span><span>StockPilot</span><span className="badge">{wallet ? `${wallet.address.slice(0, 6)}…${wallet.address.slice(-4)}` : "CONNECT WALLET"}</span><button className="wallet" onClick={connectWallet}>{wallet ? `BASE ${wallet.chainId}` : "Connect wallet"}</button></header>
    ...
    {routes && <section className="result"><p className="eyebrow">ROUTE DISCOVERY</p><h2>{routes.venue.name}</h2><p className="muted">{routes.routes.length} tokenized-stock route candidates from the registry. No route is execution-ready until pool existence, liquidity, quote, and slippage are verified.</p></section>}
